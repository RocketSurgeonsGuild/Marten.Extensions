...
namespace Rocket.Surgery.Extensions.Marten.Builders
{
    ...
    [UsedImplicitly]
    internal class MartenProjectionsConfigureOptions : IConfigureOptions<StoreOptions>
    {
            ...
            if (typeof(IProjection).IsAssignableFrom(type))
            {
                ...
                catch (Exception e)
                {
                    throw new InvalidOperationException(
                        $"Could not create an instance of '{type.FullName}'. If you need to register it through Dependency Injection use the IInlineProjection or IAsyncProjection interfaces.",
                        e
                    );
                }
            }
        ...
        public void Configure(StoreOptions options)
        {
            ...
            {
                var collection = assembly.ProjectionType == ProjectionType.Inline
                    ? options.Events.InlineProjections
                    : options.Events.AsyncProjections;
