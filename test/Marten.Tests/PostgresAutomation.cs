...
namespace Rocket.Surgery.Marten.Tests
{
    public class GitRemoteAutomation
    {
        ...
        static GitRemoteAutomation()
        {
            ...
            try
            {
                var process = Process.Start(new ProcessStartInfo("git.exe")
                {
                    Environment = { },
        ...
        static string GetRepositoryName(string str)
        {
    ...
    public static class PortRandomizer
    {
        ...
        {
            var code = Math.Abs(BitConverter.ToInt32(MD5.Create().ComputeHash(System.Text.Encoding.Default.GetBytes($"{GitRemoteAutomation.RemoteName}@{assemblyName}")), 0));
